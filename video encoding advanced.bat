...
SET /P bufsizezada=
pause
ffmpeg -i %* -an -c:v libx264 -s 1280x720 -r 23.976 -preset veryslow -crf %crfzada% -maxrate %maxratezada%k -bufsize %bufsizezada%k -profile:v high -pix_fmt yuv420p -map 0:0 -map_metadata -1 "%name% output %crfzada%crf m %maxratezada%k b %buffsizezada%k.flv"
PAUSE
